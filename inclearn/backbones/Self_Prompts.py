...
class AdaptivePromptGenerator(nn.Module):
    def __init__(self, embedding_dim, MLP_num=2, num_heads=4, attn_drop=0., proj_drop=0., more_prompts=1, attn_depth=1,
        ...
        self.hidden_1 = Mlp(embedding_dim, 512, act_layer=nn.GELU)
        self.hidden_2 = Mlp(512, 256, act_layer=nn.GELU)
        self.MLPs_out = Mlp(in_features=256, out_features=embedding_dim, act_layer=nn.GELU)
        ...
        self.norm_hidden2 = nn.LayerNorm(256)
    ...
    def forward(self, img_feat, all_tokens=True, specific=None, return_attn=False):
        ...
        #q_new = self.hidden_1(q_new)
        #q_new = self.norm_hidden(q_new)
        #q_new = self.hidden_2(q_new)
        #q_new = self.norm_hidden2(q_new)
